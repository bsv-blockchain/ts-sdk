...
export const STARK_TRANSCRIPT_DOMAIN = 'BRC97_STARK_CORE_V1_AIR'
...
function normalizePublicInputDigest (digest?: number[]): number[] {
  if (digest === undefined) {
    return sha256([...'BRC97_STARK_CORE_EMPTY_PUBLIC_INPUT'].map(c => c.charCodeAt(0)))
  }
  assertBytes(digest)
  if (digest.length !== 32) {
    throw new Error('STARK publicInputDigest must be 32 bytes')
...
function multiTraceContextDigest (
  ...
  const writer = new Writer()
  writeString(writer, 'BRC97_MULTI_TRACE_STARK_CONTEXT_V1')
