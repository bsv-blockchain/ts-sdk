...
export const BRC97_RADIX11_WINDOW_BITS = 11
export const BRC97_RADIX11_WINDOW_COUNT = 24
export const BRC97_RADIX11_FULL_WINDOWS = 23
export const BRC97_RADIX11_MAX_MAGNITUDE = 1024
export const BRC97_RADIX11_FINAL_MAX_MAGNITUDE = 8
export const BRC97_RADIX11_POINT_LIMB_BITS = 52
export const BRC97_RADIX11_POINT_LIMBS = 5
export const BRC97_RADIX11_TABLE_ROWS =
  BRC97_RADIX11_FULL_WINDOWS * (BRC97_RADIX11_MAX_MAGNITUDE + 1) +
  BRC97_RADIX11_FINAL_MAX_MAGNITUDE + 1
...
export function buildProductionRadix11PointPairTable (
  ...
  for (let window = 0; window < BRC97_RADIX11_WINDOW_COUNT; window++) {
  ...
  if (table.length !== BRC97_RADIX11_TABLE_ROWS) {
...
export function buildProductionRadix11LookupPrototype (
  ...
  const trace = buildLookupBusTrace([
    ...
    expectedLookupRequests: BRC97_RADIX11_WINDOW_COUNT,
...
export function productionRadix11LookupMetrics (
  ...
  const activeRows = BRC97_RADIX11_TABLE_ROWS + BRC97_RADIX11_WINDOW_COUNT
  ...
  return {
    ...
    selectedRows: BRC97_RADIX11_WINDOW_COUNT,
    fullWindows: BRC97_RADIX11_FULL_WINDOWS,
    finalWindowRows: BRC97_RADIX11_FINAL_MAX_MAGNITUDE + 1,
    maxMagnitude: BRC97_RADIX11_MAX_MAGNITUDE,
    finalMaxMagnitude: BRC97_RADIX11_FINAL_MAX_MAGNITUDE,
    tupleArity: LOOKUP_BUS_TUPLE_ARITY,
    proofBytes,
    proofBytesPerLookup: proofBytes === undefined
      ? undefined
      : proofBytes / BRC97_RADIX11_WINDOW_COUNT,
    overheadRowsPerLookup:
      BRC97_RADIX11_TABLE_ROWS / BRC97_RADIX11_WINDOW_COUNT
  }
}
...
export function productionRadix11TableIndex (
  ...
  if (
    ...
    window < 0 ||
    window >= BRC97_RADIX11_WINDOW_COUNT
  ) {
  ...
  if (window < BRC97_RADIX11_FULL_WINDOWS) {
    return window * (BRC97_RADIX11_MAX_MAGNITUDE + 1) + magnitude
  }
  return BRC97_RADIX11_FULL_WINDOWS *
    (BRC97_RADIX11_MAX_MAGNITUDE + 1) +
    magnitude
}
...
export function radix11Parameters (): DualBaseLookupParameters {
  return {
    ...
    pointLimbBits: BRC97_RADIX11_POINT_LIMB_BITS,
    pointLimbs: BRC97_RADIX11_POINT_LIMBS
  }
}
...
function pointLimbs (point: SecpPoint): bigint[] {
  if (point.infinity === true) {
    return new Array<bigint>(BRC97_RADIX11_POINT_LIMBS * 2).fill(0n)
  }
  return [
...
function validateFinalWindowMagnitude (digits: DualBaseSignedDigit[]): void {
  const final = digits[BRC97_RADIX11_WINDOW_COUNT - 1]
  if (
    final === undefined ||
    final.magnitude > BRC97_RADIX11_FINAL_MAX_MAGNITUDE
  ) {
...
function radix11WindowMaxMagnitude (window: number): number {
  return window < BRC97_RADIX11_FULL_WINDOWS
...
function multiplyByRadix (point: SecpPoint): SecpPoint {
  let out = point
  for (let i = 0; i < BRC97_RADIX11_WINDOW_BITS; i++) {
    out = pointDouble(out)
  }
  return out
}
