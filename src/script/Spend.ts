...
export interface ScriptMacroDefinition {
  ...
  apply: (spend: Spend, match: ScriptMacroMatch) => boolean
...
function applyHash256Reverse32 (spend: Spend): boolean {
...
function applyCompilablePreamble (spend: Spend): boolean {
  if (spend.stack.length < 1) return false
  ...
  const cache = (spend as any).sigHashCache as SignatureHashCache | undefined
...
function applySplit1xRun (spend: Spend, match: ScriptMacroMatch): boolean {
...
function applySwapCatRun (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
  if (!Number.isInteger(count) || count < 1) return false
...
function applyFromAltStackRun (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
  if (!Number.isInteger(count) || count < 1) return false
...
function applySwapToAltStackRun (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
  if (!Number.isInteger(count) || count < 1) return false
...
function applySplitDrop (spend: Spend): boolean {
...
function applySplitNip (spend: Spend): boolean {
  if (spend.stack.length < 2) return false
  const posBuf = spend.stack[spend.stack.length - 1]
  const dataToSplit = spend.stack[spend.stack.length - 2]
  if (!Array.isArray(posBuf) || !Array.isArray(dataToSplit)) return false
...
function applyConstSplitDrop (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
...
function applyConstSplitNip (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
  if (!Number.isInteger(count) || count < 0) return false
  if (spend.stack.length < 1) return false
  if (spend.stackMem + 1 > spend.memoryLimit) return false
...
function applyDepthPushSub1SubPick (spend: Spend, match: ScriptMacroMatch): boolean {
...
function apply2DropRun (spend: Spend, match: ScriptMacroMatch): boolean {
  const count = typeof match.meta?.count === 'number' ? match.meta.count : 0
  if (!Number.isInteger(count) || count < 1) return false
...
const DEFAULT_MACROS: ReadonlyArray<ScriptMacroDefinition> = Object.freeze([
  Object.freeze({
    name: 'compilable-preamble',
    match: matchCompilablePreamble,
    apply: (spend: Spend) => applyCompilablePreamble(spend),
  ...
  Object.freeze({
    ...
    apply: (spend: Spend) => applyHash256Reverse32(spend),
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applySplit1xRun(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applySwapCatRun(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applyFromAltStackRun(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applySwapToAltStackRun(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend) => applySplitDrop(spend),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    name: 'split-nip',
    match: matchSplitNip,
    apply: (spend: Spend) => applySplitNip(spend),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applyConstSplitDrop(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applyConstSplitNip(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    ...
    apply: (spend: Spend, match: ScriptMacroMatch) => applyDepthPushSub1SubPick(spend, match),
    skipWhenNotExecuting: true,
  ...
  Object.freeze({
    name: '2drop-run',
    match: match2DropRun,
    apply: (spend: Spend, match: ScriptMacroMatch) => apply2DropRun(spend, match),
    skipWhenNotExecuting: true,
...
export default class Spend {
  ...
  stackMem: number
  altStackMem: number
  private macros: ScriptMacroDefinition[]
  private macroLookup: Map<string, ScriptMacroDefinition>
  private macroOwnerId: number
  private macroStats: Map<string, number>
   ...
   * @param {ScriptMacroDefinition[]} [params.macros] - Optional macros to scan/apply (custom macros run before defaults).
   * @param {boolean} [params.enableDefaultMacros=true] - Whether to enable the built-in macro set.
   *
   * @example
  ...
  constructor (params: {
    ...
    lockTime: number
    memoryLimit?: number
    macros?: ScriptMacroDefinition[]
    enableDefaultMacros?: boolean
  }) {
    ...
    this.stackMem = 0
    this.altStackMem = 0
    const enableDefaultMacros = params.enableDefaultMacros ?? true
    const customMacros = params.macros ?? []
    this.macros = enableDefaultMacros
      ? [...customMacros, ...DEFAULT_MACROS]
      : customMacros.slice()
    this.macroLookup = new Map(this.macros.map(macro => [macro.name, macro]))
    this.macroOwnerId = ++macroOwnerCounter
    this.macroStats = new Map()
    this.sigHashCache = { hashOutputsSingle: new Map() }
    this.reset()
    this.scanScriptForMacros(this.unlockingScript)
    this.scanScriptForMacros(this.lockingScript)
  }
  ...
  reset (): void {
    ...
    this.stackMem = 0
    this.altStackMem = 0
    this.macroStats.clear()
    this.sigHashCache = { hashOutputsSingle: new Map() }
  }

  getMacroStats (): ReadonlyMap<string, number> {
    return this.macroStats
  }

  private recordMacroHit (name: string): void {
    const count = this.macroStats.get(name) ?? 0
    this.macroStats.set(name, count + 1)
  }

  private scanScriptForMacros (script: Script): void {
  ...
  step (): boolean {
    ...
    const macroTag = operation.macro
    if (
      macroTag != null &&
      macroTag.ownerId === this.macroOwnerId &&
      macroTag.role === 'start' &&
      macroTag.length > 0
    ) {
      const macro = this.macroLookup.get(macroTag.name)
      if (macro != null) {
        if (isScriptExecuting) {
          const applied = macro.apply(this, { length: macroTag.length, meta: macroTag.meta })
          if (applied) {
            this.recordMacroHit(macro.name)
            this.programCounter += macroTag.length
            return true
          }
        } else if (macro.skipWhenNotExecuting !== false) {
          this.programCounter += macroTag.length
          return true
        }
      }
