...
The following pieces remain useful as tests, oracles, or historical prototypes,
but are not the production Method 2 proving architecture:

| Component | Status | Reason |
| --- | --- | --- |
| Row-serialized Method 2 VM | Prototype/oracle | Not the production proving shape. |
| V2 6-bit fixed-window path | Prototype/oracle | Superseded by signed radix-11 dual-base lookup. |
| Old row-expanded scalar control | Prototype/oracle | Not used by the whole production proof. |
| 240-subproof affine EC field-op bundle | Oracle | Replaced by the hardened EC accumulator AIR. |
| Compact HMAC AIR | Oracle | Correct relation, but no longer the production HMAC path. |
| Standalone segment proofs | Debug/metrics only | Real production users should run the whole statement proof. |
| Standalone lookup/equality bus prototype | Debug/metrics only | Production uses segment-local buses and shared equality domain. |
| Masked-opening endpoint links | Retired | Not sound for private equality across separately masked traces. |
